...
class MedicationPrintManager {
    ...
    private func presentPrintInterface(with pdfData: Data, jobName: String) async {
        ...
        printInfo.duplex = .none // Single-sided printing for labels
        
        printController.printInfo = printInfo
        printController.printingItem = pdfData
        
        // Find the current window and present the print interface
    ...
    func sharePDF(for medication: DispencedMedication) async {
        ...
        do {
            ...
            // Find the current window and present the share sheet
            guard let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
                  let window = windowScene.windows.first,
                  let rootViewController = window.rootViewController else {
